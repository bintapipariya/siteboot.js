...
ServerObject.prototype.search = function(opts, context){
	...
	if(opts){
		...
		Object.keys(opts).map(function(x){
			console.debug(JSON.stringify(Object.keys(self._table.rawAttributes))); 
...
ServerObject.prototype.browse = function(ids, opts){
	...
	console.debug("Using where "+where); 
	...
	if(where){
		console.debug("Selecting with where conditional: "+where); 
		self._table.findAll(where).success(function(objs){
			done(objs); 
		}); 
	} else {
		console.debug("Selecting all records"); 
